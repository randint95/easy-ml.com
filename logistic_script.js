...
elements.regressionBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // ロジスティック回帰分析を実行するAPIを呼ぶ
        const result = await callApi('/api/logistic', payload);
...
function renderVariableSelection(columns) {
    ...
    columns.forEach((col, index) => {
        ...
        xCheck.addEventListener('change', () => { 
            if(xCheck.checked && yRadio.checked) yRadio.checked = false; 
...
function checkStep3Ready() {
    ...
    elements.regressionBtn.disabled = !(sel.target && sel.features.length > 0);
}

// サーバーから返ってきた分析結果をHTMLに整形して表示する関数（ロジスティック回帰用）
function renderResult(data) {
    ...
    let html = `
        <p><strong>モデルの正解率 (Accuracy):</strong> ${(data.accuracy * 100).toFixed(2)} % ${standardizeBadge}</p>
        <p>予測対象クラス: ${data.classes.join(', ')}</p>
        <h3>係数 (分類への影響度)</h3>
    `;
    
    // 標準化されている場合は、係数の意味が変わるため注釈を入れる
    if (data.standardized) {
        html += '<p style="font-size: 0.9em; color: #7f8c8d;">※標準化されているため、係数の絶対値が大きいほど分類への影響度が大きくなります。</p>';
    }
